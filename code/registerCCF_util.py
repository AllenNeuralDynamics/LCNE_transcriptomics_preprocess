...
def get_adjusted_points(df, name_df, details):
    ...
    def _to_series(x):
        t = transform(triangulation, x['rescale_x'], x['rescale_y'])
        if t is None:
            return pd.Series([np.nan, np.nan], index=['adjusted_x','adjusted_y'], dtype=float)
        return pd.Series(t, index=['adjusted_x','adjusted_y'], dtype=float)

    return df.apply(_to_series, axis=1)